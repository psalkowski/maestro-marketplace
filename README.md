...
Claude Code plugins for provider-agnostic docs, spec workflows, and Conductor integration. Install `maestro` and run `/maestro:setup` to onboard a project — it sets up the rest for you.
...
| --- | --- | --- |
| maestro | Start here. `/maestro:setup` interviews you (docs location, Conductor, spec workflow) and installs + configures the right plugins in order | 0.1.1 |
...
| conductor-kit | Conductor integration: discovery-first GUI/terminal-parity action skills (create-pr, code-review, fix-errors, resolve-conflicts, rename-branch, address-cr), grounded in Conductor's real defaults; personal-layer prompt overrides; workspace setup script with docs seeding | 0.2.0 |
