...
`graphify-out/graph.json` indexes this repo (AST symbols + indexed docs). It is a **symbol navigator, not a search engine** — it complements grep, it does not replace it. Workflow: harvest a real symbol name → `graphify` to map relationships → Read only the files you'll change. For a multi-hop sweep, delegate to the Explore agent rather than crawling here (see the delegation bullets).
...
- **Re-entry:** the moment grep/Read surfaces a function/class/component name, the next relationship question goes to `graphify explain "<thatName>"` — not another grep, not a full-file Read.
...
- **Delegate larger explores.** A quick 1–2 hop lookup is cheapest inline; a multi-hop sweep should go to the graph-aware Explore agent (`graphify-kit:Explore`, or the project `.claude/agents/explore.md`, on haiku) — **never the bare native `Explore`** (it greps blindly), and a premium (Opus/Fable) main loop shouldn't crawl 10+ steps itself. Subagents don't inherit this CLAUDE.md, so prepend the protocol (and a ready-to-run `graphify explain "<Symbol>"`) when dispatching any OTHER agent type.
- **A delegated explore returns evidence, not truth.** You get a findings map (`file:line` + `read`/`explain`/`grep`/`inferred` tags + a load-bearing subset), not a vetted answer — you author the response and re-read the load-bearing anchors before asserting (a wrong, unverified handoff costs more than no delegation). _Opt-in, high-stakes:_ dispatch a second haiku Explore to independently refute the load-bearing claims and keep only the survivors.
