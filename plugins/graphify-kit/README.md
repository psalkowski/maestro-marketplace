...
3. **Delivery to every context** — a `CLAUDE.md` section for the main loop, a project-level `Explore` override (the built-in's description, graph protocol in the body) for subagents (they don't inherit CLAUDE.md, and hook nudges don't reach them), and PreToolUse nudges that fire at exactly the moment the agent reaches for grep.
...
| `templates/`              | The CLAUDE.md protocol section and the Explore agent — single sources the setup skill copies into the project; no plugin-level agent is registered                                                   |
